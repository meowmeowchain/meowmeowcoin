...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Meowmeowcoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Meowmeowcoin server stopping";
}
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use meowmeowcoind, or the -server option to meowmeowcoin-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
              "It is recommended you use the following random password:\n"
              "rpcuser=meowmeowcoinrpc\n"
              "rpcpassword=%s\n"
...
std::string HelpExampleCli(string methodname, string args){
    return "> meowmeowcoin-cli " + methodname + " " + args + "\n";
}
